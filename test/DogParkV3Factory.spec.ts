...
describe('DogParkV3Factory', () => {
  const [wallet, other] = waffle.provider.getWallets()

  let factory: DogParkV3Factory
  let poolBytecode: string
  const fixture = async () => {
    const factoryFactory = await ethers.getContractFactory('DogParkV3Factory')
    return (await factoryFactory.deploy()) as DogParkV3Factory
  }
  ...
  before('load pool bytecode', async () => {
    poolBytecode = (await ethers.getContractFactory('DogParkV3Pool')).bytecode
  })
  ...
  async function createAndCheckPool(
    ...
    const poolContractFactory = await ethers.getContractFactory('DogParkV3Pool')
